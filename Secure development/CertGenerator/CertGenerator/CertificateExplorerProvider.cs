...
namespace CertGenerator
{
    public class CertificateExplorerProvider
    {
        ...
        private List<X509CertificateWrapper> certList;
        ...
        public List<X509CertificateWrapper> Certificates 
        {  
            get { return certList; } 
        }
        public void LoadCertificates()
        {
            certList = new List<X509CertificateWrapper>();
            foreach (string store in certStores)
            {
                certList.AddRange(LoadStore(store));
            }
        }
        private List<X509CertificateWrapper> LoadStore(string storeName)
        {
        ...
        private List<X509CertificateWrapper> CertificatesToView(X509Certificate2Collection certificates, string groupName)
        {
            List<X509CertificateWrapper> certList = new List<X509CertificateWrapper>();
            foreach(X509Certificate2 cert in certificates)
            {
                ...
                if (requirePrivateKey)
                {
                    ...
                    {
                        certList.Add(new X509CertificateWrapper(cert, groupName, groupDesc));
                    }
                }
                else
                {
